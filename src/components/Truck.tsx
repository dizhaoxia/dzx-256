  ...
  return (
    <div
      ...
      className={`relative transition-all duration-300 ${
        truck.matched ? 'cursor-default' : 'cursor-pointer'
      } ${animateSuccess ? 'animate-bounce-hard' : ''} ${
        clickPulse && !truck.matched ? 'scale-[0.97]' : ''
      }`}
    >
      <div
        className={`relative rounded-3xl p-4 md:p-5 transition-all duration-300 ${
          truck.matched
            ...
            : 'border-4 border-transparent hover:scale-[1.02] hover:shadow-2xl'
        } ${clickPulse && !truck.matched && !isSelected ? 'ring-4 ring-yellow-300 ring-opacity-60' : ''}`}
        style={{
      ...
      >
        ...
        <div className="mt-8 relative">
          ...
          >
            ...
            {truck.matched && (
              <g>
                {Array.from({ length: truck.target }).map((_, i) => (
                  <text key={i} x={30 + i * 35} y="68" fontSize="28" textAnchor="middle">
            ...
            <path
              d="M150 25 L200 25 Q220 25 225 45 L225 75 Q225 82 218 82 L150 82 Z"
              fill={`url(#cabGrad-${truck.id})`}
              stroke="#000"
              strokeWidth="2"
              strokeOpacity="0.2"
            />
        ...
        <div className="mt-3 text-center">
          <div
            className={`text-sm md:text-base font-bold ${
              truck.matched ? 'text-green-700' : isSelected ? 'text-yellow-700' : 'text-gray-700'
            }`}
          >
            {truck.matched ? (
        ...
        {truck.matched && (
          <div className="absolute top-2 right-2 text-2xl md:text-3xl animate-bounce-soft">⭐</div>
        )}

        {isSelected && !truck.matched && (
          <div className="absolute -top-2 -right-2 w-8 h-8 md:w-10 md:h-10 bg-gradient-to-br from-green-400 to-emerald-500 rounded-full flex items-center justify-center text-white text-sm md:text-base font-black shadow-lg animate-pop-in border-2 border-white">
            ✓
          </div>
        )}
      </div>
